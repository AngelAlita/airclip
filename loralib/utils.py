        ...
        for i, block in enumerate(text_encoder.layers):
            ...
            if i in indices:
                for name, submodule in block.named_children():
                    if 'attn' in name:
                        for attn_name, attn_submodule in submodule.named_children():
                            # 检查是否为Linear层
                            if isinstance(attn_submodule, nn.Linear):
                                if 'q' in attn_name and 'q' in args.params:
                                    new_q = LinearLoRA(attn_submodule,r=args.r,lora_alpha=args.lora_alpha, lora_dropout=args.dropout_rate)
                                    setattr(submodule,attn_name, new_q)
                                    list_lora_layers.append(new_q)
                                elif 'k' in attn_name and 'k' in args.params:
                                    new_k = LinearLoRA(attn_submodule,r=args.r,lora_alpha=args.lora_alpha, lora_dropout=args.dropout_rate)
                                    setattr(submodule, attn_name, new_k)
                                    list_lora_layers.append(new_k)
                                elif 'v' in attn_name and 'v' in args.params:
                        ...
                        for mlp_name, mlp_submodule in submodule.named_children():
                            if 'fc1' in mlp_name and 'fc1' in args.params:
                                new_fc1 = LinearLoRA(mlp_submodule,r=args.r,lora_alpha=args.lora_alpha, lora_dropout=args.dropout_rate)
                                setattr(submodule,mlp_name, new_fc1)
                                list_lora_layers.append(new_fc1)
                            elif 'fc2' in mlp_name and 'fc2' in args.params:
                                new_fc2 = LinearLoRA(mlp_submodule,r=args.r,lora_alpha=args.lora_alpha, lora_dropout=args.dropout_rate)
                                setattr(submodule,mlp_name, new_fc2)
                                list_lora_layers.append(new_fc2)
